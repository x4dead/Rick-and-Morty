...
/// [_SearchDelegate.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [_SearchDelegate] can only be associated with one active [showSearchBar]
/// call. Call [_SearchDelegate.close] before re-using the same delegate instance
/// for another [showSearchBar] call.
///
...
/// The animation provided by [_SearchDelegate.transitionAnimation] can be used
...
/// See also:
///
///  * [_SearchDelegate] to define the content of the search page.
Future<T?> showSearchBar<T>({
  required BuildContext context,
  required _SearchDelegate<T> delegate,
  String? query = '',
...
// final characters = FutureProvider.family<List<RestSingleCharacter>?, String>(
//     (ref, text) async {
//   return ref.watch(River.charactersPod.notifier).searchCharacters(text);
// });

class CustomSearchDelegate extends _SearchDelegate {
  ...
  // set searchFieldLabel => '';
  @override
  List<Widget>? buildActions(BuildContext context) {
    return [
      IconButton(
        icon: const Icon(Icons.close_rounded, color: AppColors.colorFFFFFF),
        onPressed: () {
          query = '';
        },
      )
  ...
  Widget? buildLeading(BuildContext context) {
    return IconButton(
      icon: const Icon(Icons.arrow_back_rounded, color: AppColors.colorFFFFFF),
      onPressed: () {
        close(context, null);
      },
    );
  ...
  Widget buildResults(BuildContext context) {
    final characters =
        ref.watch(River.charactersPod.notifier).searchCharacters(query);
    return Column(
      children: [
        const Divider(height: 1, color: AppColors.color152A3A),
        kSBH23,
        Row(
          children: [
            kSBW16,
        ...
        kSBH24,
        // Expanded(
        //   child: ref.watch(characters(query)).when(
        //         loading: () => Text('data'),
        //         data: (data) {
  ...
  @override
  String? get searchFieldLabel => 'Найти персонажа';
}

/// Delegate for [showSearchBar] to define the content of the search page.
///
/// The search page always shows an [AppBar] at the top where users can
/// enter their search queries. The buttons shown before and after the search
/// query text field can be customized via [_SearchDelegate.buildLeading]
/// and [_SearchDelegate.buildActions]. Additionally, a widget can be placed
/// across the bottom of the [AppBar] via [_SearchDelegate.buildBottom].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [_SearchDelegate.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [_SearchDelegate.buildResults].
///
/// [_SearchDelegate.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [_SearchDelegate.showResults]
/// and you can go back to showing the suggestions by calling
/// [_SearchDelegate.showSuggestions].
///
/// Once the user has selected a search result, [_SearchDelegate.close] should be
/// called to remove the search page from the top of the navigation stack and
/// to notify the caller of [showSearchBar] about the selected search result.
///
/// A given [_SearchDelegate] can only be associated with one active [showSearchBar]
/// call. Call [_SearchDelegate.close] before re-using the same delegate instance
/// for another [showSearchBar] call.
...
/// ## Handling emojis and other complex characters
/// {@macro flutter.widgets.EditableText.onChanged}
abstract class _SearchDelegate<T> {
  ...
  /// communicate the result back to the initial caller of [showSearchBar].
  Widget? buildResults(BuildContext context);
  ...
  /// to [showSearchBar] that launched the search initially.
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [_SearchDelegate.buildSuggestions].
  // suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [_SearchDelegate.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final _SearchDelegate<T> delegate;
...
class _SearchPageState<T> extends State<_SearchPage<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    //     child: widget.delegate.buildSuggestions(context) ?? Text('data'),
    //   );
    //   break;
    // case _SearchBody.results:
    body = KeyedSubtree(
      key: const ValueKey<_SearchBody>(_SearchBody.results),
      child: widget.delegate.buildResults(context) ?? kNothing,
    );
    //   break;
    // case null:
    //   break;
        ...
        child: Scaffold(
          ...
          appBar: AppBar(
            backgroundColor: focusNode.hasFocus
                ? AppColors.color152A3A
                : AppColors.color0B1E2D,
            leading: widget.delegate.buildLeading(context),
            title: TextField(
              controller: widget.delegate._queryTextController,
              focusNode: focusNode,
              style:
                  widget.delegate.searchFieldStyle ?? AppTextStyle.w400s16white,
              textInputAction: widget.delegate.textInputAction,
              keyboardType: widget.delegate.keyboardType,
              onSubmitted: (String _) => widget.delegate.showResults(context),
              decoration: InputDecoration(
                  hintText: searchFieldLabel, hintStyle: AppTextStyle.w400s16
